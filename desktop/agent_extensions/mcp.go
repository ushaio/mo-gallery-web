...
func initializeAndListTools(ctx context.Context, runtime *mcpRuntime) ([]MCPTool, error) {
	...
	runtime.mu.Unlock()
	if !initialized {
		if _, err := runtime.request(ctx, "initialize", map[string]any{"protocolVersion": "2024-11-05", "capabilities": map[string]any{}, "clientInfo": map[string]any{"name": "mo-gallery-desktop", "version": "0.8.1"}}); err != nil {
			return nil, err
		}
