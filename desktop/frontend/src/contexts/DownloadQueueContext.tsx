...
import { DownloadCloudPhotoToLocalLibrary } from '../../wailsjs/go/main/App'
import type { local_library } from '../../wailsjs/go/models'

export type DownloadTaskStatus = 'pending' | 'downloading' | 'importing' | 'completed' | 'failed'
...
interface DownloadQueueContextType {
  ...
  startDownload: (photoId: string, destination: string, libraryPath: string, fileName: string, fileSize: number) => void
...
export function DownloadQueueProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const unsubscribe = EventsOn('download:progress', (event: {
      ...
      if (event.phase === 'completed') {
        patchTask(taskId, { status: 'completed', progress: 100, error: undefined })
      } else if (event.phase === 'failed') {
        patchTask(taskId, { status: 'failed', progress: 0, error: event.error || '下载失败' })
      } else if (event.phase === 'downloading') {
        patchTask(taskId, { status: 'downloading', progress: event.progress, error: undefined })
  ...
  const startDownload = useCallback((photoId: string, destination: string, libraryPath: string, fileName: string, fileSize: number) => {
    ...
    void (async () => {
      try {
        const results = await DownloadCloudPhotoToLocalLibrary(photoId, destination, libraryPath)
  ...
  return (
    <DownloadQueueContext.Provider value={{ tasks, isDownloading, startDownload, removeTask, clearCompleted }}>
      {children}
