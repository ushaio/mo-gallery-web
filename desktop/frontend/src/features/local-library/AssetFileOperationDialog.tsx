      ...
      <div role="dialog" aria-modal="true" aria-labelledby="asset-file-operation-title" className="relative w-full max-w-md rounded-xl border bg-background p-5 shadow-2xl">
        ...
        <form className="mt-5 space-y-4" onSubmit={(event) => { event.preventDefault(); submit() }}>
          {mode === 'rename' ? (
            ...
            <div>
              <label className="text-[11px] font-medium" htmlFor="local-library-asset-destination">{copy.destinationFolder}</label>
              <select id="local-library-asset-destination" value={destinationFolder} onChange={(event) => setDestinationFolder(event.target.value)} disabled={busy} className="mt-2 h-9 w-full rounded-md border bg-input px-3 text-xs outline-none focus:ring-1 focus:ring-ring">
                {destinations.map((destination) => <option key={destination.value || 'root'} value={destination.value}>{destination.label}</option>)}
              </select>
            </div>
          )}
