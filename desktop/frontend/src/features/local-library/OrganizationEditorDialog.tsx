...
type EditorTarget =
  ...
  | { kind: 'group', item?: CollectionGroup, parentId?: string }
...
export function OrganizationEditorDialog({ target, groups, copy, busy, onClose, onSubmit }: Props) {
  ...
  const currentParent = target.kind === 'collection'
    ...
    : target.kind === 'group'
      ...
      : undefined
  const [parentId, setParentId] = useState(currentParent || (target.kind !== 'tag' ? target.parentId : undefined) || '')
  ...
  const title = target.kind === 'tag'
    ...
    : target.kind === 'collection'
      ...
      : (item ? copy.renameCollectionGroup : copy.newCollectionGroup)
    ...
    <div className="fixed inset-0 z-[130] flex items-center justify-center p-5">
      ...
      <div role="dialog" aria-modal="true" className="relative w-full max-w-sm rounded-xl border bg-background p-5 shadow-2xl">
        ...
        <form className="mt-5 space-y-4" onSubmit={(event) => { event.preventDefault(); submit() }}>
          ...
          {target.kind !== 'tag' && <label className="block text-[11px] font-medium">{copy.collectionParent}<select value={parentId} onChange={(event) => setParentId(event.target.value)} disabled={busy} className="mt-2 h-9 w-full rounded-md border bg-input px-3 text-xs"><option value="">{copy.collectionRoot}</option>{groups.filter((group) => !excluded.has(group.id)).map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label>}
