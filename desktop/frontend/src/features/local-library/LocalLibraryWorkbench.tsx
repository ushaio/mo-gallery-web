...
import {
  LocalLibraryGuide,
  LOCAL_LIBRARY_GUIDE_SEEN_KEY,
} from "./LocalLibraryGuide";
...
import {
  ...
  type UploadSettings,
} from "@/components/admin/ImageUploadSettingsModal";
import {
...
function scanLabel(snapshot: LibrarySnapshot, copy: LocalLibraryCopy) {
  if (snapshot.state === "suspended" || snapshot.scan.state === "suspended")
    return copy.librarySuspended;
  if (snapshot.state === "repair_required") return copy.libraryIdentityMismatch;
  switch (snapshot.scan.state) {
    case "running":
      return copy.scanning;
    case "paused":
      return copy.scanPaused;
    case "failed":
      return snapshot.scan.error
        ? `${copy.scanFailed}: ${snapshot.rootPath} - ${snapshot.scan.error}`
        : `${copy.scanFailed}: ${snapshot.rootPath}`;
    default:
      return copy.scanDone;
  }
}

function runResultsMessage<T extends { status: string; error?: string }>(
  results: T[],
  success: string,
  partial: string,
) {
  ...
  if (failed.length === 0) toast.success(success);
    ...
    toast.error(`${partial} (${failed.length}/${results.length})`, {
...
export function LocalLibraryWorkbench({
  ...
  const foldersOpen = useLibrarySections(
    (state) => state.sections.localFolders,
  );
  const colorsOpen = useLibrarySections(
    (state) => state.sections.localColors ?? true,
  );
  const ratingsOpen = useLibrarySections(
    (state) => state.sections.localRatings ?? true,
  );
  const toggleSection = useLibrarySections((state) => state.toggleSection);
  const photoViewMode = usePreferences((state) => state.photoViewMode);
  const setPhotoViewMode = usePreferences((state) => state.setPhotoViewMode);
  const photoGridSize = usePreferences((state) => state.photoGridSize);
  ...
  const [removeMissingAsset, setRemoveMissingAsset] =
    useState<LocalAsset | null>(null);
  const [dropTargetFolder, setDropTargetFolder] = useState<string | null>(null);
  const [assetDropTargetFolder, setAssetDropTargetFolder] = useState<
    string | null
  >(null);
  const [draggedFolderPath, setDraggedFolderPath] = useState<string | null>(
    null,
  );
  const [folderDropTarget, setFolderDropTarget] = useState<string | null>(null);
  const viewMode = photoViewMode;
  const gridSize = Math.min(280, Math.max(120, photoGridSize));
  ...
  const [pendingImportDestination, setPendingImportDestination] = useState<
    string | null
  >(null);
  ...
  const [backupOperation, setBackupOperation] = useState<
    "create" | "restore" | null
  >(null);
  const [createFolderParent, setCreateFolderParent] =
    useState<FolderTarget | null>(null);
  const [organizeFolderTarget, setOrganizeFolderTarget] = useState<{
    target: FolderTarget;
    mode: "rename" | "move";
  } | null>(null);
  const [folderMovePlan, setFolderMovePlan] =
    useState<FolderFileOperationPlan>();
  const [deleteFolderTarget, setDeleteFolderTarget] =
    useState<FolderTarget | null>(null);
  const [propertiesFolder, setPropertiesFolder] = useState<FolderTarget | null>(
    null,
  );
  const [folderProperties, setFolderProperties] = useState<
    FolderProperties | undefined
  >();
  const [folderDeletionPreview, setFolderDeletionPreview] = useState<
    FolderDeletionPreview | undefined
  >();
  const [folderDeletionPreviewLoading, setFolderDeletionPreviewLoading] =
    useState(false);
  const [folderDeletionPreviewError, setFolderDeletionPreviewError] =
    useState<string>();
  ...
  const [restoreFolderTarget, setRestoreFolderTarget] =
    useState<FolderTrashEntry | null>(null);
  const [permanentFolderTarget, setPermanentFolderTarget] =
    useState<FolderTrashEntry | null>(null);
  ...
  const [assetFileOperation, setAssetFileOperation] = useState<
    | { mode: "rename"; asset: LocalAsset }
    | { mode: "move"; assetIds: string[] }
    | null
  >(null);
  ...
  const [quickUploadSettings, setQuickUploadSettings] =
    useState<UploadSettings>(() =>
      loadLocalLibraryUploadSettings(
  ...
  const [organizationEditor, setOrganizationEditor] =
    useState<OrganizationEditorTarget | null>(null);
  const [organizationDelete, setOrganizationDelete] =
    useState<OrganizationDeleteTarget | null>(null);
  const [organizationBusy, setOrganizationBusy] = useState(false);
  const [guideOpen, setGuideOpen] = useState(false);
  const [selectedAssetIds, setSelectedAssetIds] = useState<string[]>([]);
  const existingAssetIdSet = useMemo(
    () => new Set(existingAssetIds),
    [existingAssetIds],
  );
  const [selectedQueryToken, setSelectedQueryToken] = useState<string | null>(
    null,
  );
  ...
  const folderTreeAssetCounts = useMemo(() => {
    ...
    const deepestFirst = folders.toSorted(
      (left, right) =>
    ...
    for (const item of deepestFirst) {
      ...
      if (item.parentId)
        byId.set(item.parentId, (byId.get(item.parentId) || 0) + count);
    }
    return byId;
  }, [folders]);
  ...
  const refreshSnapshot = useCallback(async () => {
    try {
      onSnapshot(await localLibraryApi.snapshot());
    } catch {
  ...
  const reloadFolders = useCallback(async () => {
    try {
      setFolders(await localLibraryApi.listFolders());
    } catch (error) {
  ...
  const reloadFavoriteCount = useCallback(async () => {
    try {
      const result = await localLibraryApi.listAssets({
        availability: "active",
        favoritesOnly: true,
        limit: 1,
      });
      ...
      /* the active asset request reports session errors */
    }
  }, []);

  const reloadOrganization = useCallback(async () => {
    try {
      const [nextTags, nextGroups, nextCollections] = await Promise.all([
        localLibraryApi.listTags(),
        localLibraryApi.listCollectionGroups(),
        localLibraryApi.listCollections(),
      ]);
      setTags(nextTags);
  ...
  const openFolderFromContent = useCallback(
    (nextFolder: FolderItem) => {
  ...
  const selectGridAsset = useCallback(
    (asset: LocalAsset, intent?: { toggle?: boolean; range?: boolean }) => {
      if (selectionMode) {
      ...
      if (intent?.toggle) {
        setSelectedAssetIds((current) => {
          ...
          selectAsset(
            page.items.find((item) => item.id === nextSelectedId) ?? null,
          );
          return next;
        });
  ...
  const reloadTrashedFolders = useCallback(async () => {
    ...
    try {
      setTrashedFolders(await localLibraryApi.listTrashedFolders());
    } catch (error) {
      toast.error(parseLocalLibraryError(error).message);
    } finally {
      setTrashedFoldersLoading(false);
    }
  }, []);
  ...
  useCachedPageEffect(() => {
    ...
    localLibraryApi
      ...
      .finally(() => {
        activeAssetRequestsRef.current = Math.max(
          0,
          activeAssetRequestsRef.current - 1,
        );
        if (activeAssetRequestsRef.current === 0)
          previewStatusOverridesRef.current.clear();
    ...
    if (selectionMode && onSelectionChange)
      onSelectionChange(
        selectedAssets.filter(
          (asset) => asset.availability === "active" && isPhotoAsset(asset),
        ),
      );
  }, [onSelectionChange, selectedAssets, selectionMode]);
  const selectedCount = selectedQueryToken
    ? selectedQueryTotal
    : selectedAssetIds.length;
  const allLoadedSelected =
    page.items.length > 0 &&
    page.items.every((item) => selectedAssetIds.includes(item.id));
  const selectedUploadAssets = selectedAssets.filter(
    (asset) => asset.availability === "active" && isPhotoAsset(asset),
  );
  const selectedUploadedCount = selectedAssets.filter(
    (asset) => asset.isUploaded,
  ).length;
  const selectedFolderBatchCount = new Set(
    selectedAssets
      .filter((asset) => asset.trashEntryKind === "folder")
      .map((asset) => asset.trashEntryId || asset.id),
  ).size;
  ...
  useCachedPageEffect(() => {
    void reloadFolders();
  }, [reloadFolders, refreshKey]);
  useCachedPageEffect(() => {
    void reloadFavoriteCount();
  }, [reloadFavoriteCount, refreshKey]);
  useCachedPageEffect(() => {
  ...
  useEffect(() => {
    ...
    localLibraryApi
      .folderProperties(propertiesFolder.relativePath)
      .then((properties) => {
        if (!disposed) setFolderProperties(properties);
      })
      .catch((error) => {
        if (!disposed) {
          toast.error(parseLocalLibraryError(error).message);
      ...
      .finally(() => {
        if (!disposed) setFolderPropertiesLoading(false);
      });
    return () => {
      disposed = true;
    };
  ...
  useEffect(() => {
    ...
    localLibraryApi
      .previewFolderDeletion(deleteFolderTarget.relativePath)
      .then((preview) => {
        if (!disposed) setFolderDeletionPreview(preview);
      })
      .catch((error) => {
        if (!disposed)
          setFolderDeletionPreviewError(parseLocalLibraryError(error).message);
      })
      .finally(() => {
        if (!disposed) setFolderDeletionPreviewLoading(false);
      });
    return () => {
      disposed = true;
    };
  ...
  useEffect(() => {
    const unsubscribe = EventsOn("local-library:event", (rawEvent: unknown) => {
      ...
      if (
        event.kind === "asset_preview_updated" &&
        event.assetId &&
        event.previewStatus
      ) {
        ...
        const patchPreviewStatus = (asset: LocalAsset) =>
          asset.id === assetId ? { ...asset, previewStatus } : asset;
        setPage((current) => ({
          ...current,
          items: current.items.map(patchPreviewStatus),
        }));
  ...
  const runImport = useCallback(
    async (paths: string[], destinationFolder: string) => {
      const results = await localLibraryApi.importFiles(
        paths,
        destinationFolder,
      );
  ...
  const importPaths = useCallback(
    async (paths: string[], destinationFolder = folder) => {
  ...
  const chooseImportMode = useCallback(
    async (mode: LocalLibraryImportMode) => {
  ...
  useEffect(() => {
    OnFileDrop((x, y, paths) => {
      ...
      const target = element?.closest<HTMLElement>(
        "[data-local-library-import-folder]",
      );
      if (!target) {
        toast.error(
          element?.closest("[data-local-library-logical-target]")
            ? copy.externalDropLogicalRejected
            : copy.externalDropUnavailable,
        );
        return;
      }
      void importPathsRef.current(
        paths || [],
        target.dataset.localLibraryImportFolder || "",
      );
    }, true);
    ...
    const updateDropTarget = (event: globalThis.DragEvent) => {
      const target = document
        ...
        ?.closest<HTMLElement>("[data-local-library-import-folder]");
      event.dataTransfer!.dropEffect = target ? "copy" : "none";
      setDropTargetFolder(
        target ? target.dataset.localLibraryImportFolder || "" : null,
      );
    };
  ...
  const loadMore = useCallback(async () => {
    ...
    activeAssetRequestsRef.current += 1;
    try {
      const next = await localLibraryApi.listAssets({
        ...query,
  ...
  const chooseFiles = async () => {
    try {
      await importPaths(await localLibraryApi.selectImportFiles());
    } catch (error) {
      toast.error(parseLocalLibraryError(error).message);
    }
  };
  ...
  const loadBackups = useCallback(async () => {
    ...
    try {
      setBackupOverview(await localLibraryApi.backups());
    } catch (error) {
      toast.error(parseLocalLibraryError(error).message);
    } finally {
  ...
  const saveAsset = useCallback(
    (
      ...
      const operation = saveQueueRef.current.then(async () => {
        await localLibraryApi.updateAsset(
          assetId,
          patch.displayTitle || "",
          patch.notes || "",
          patch.rating,
        ...
        if (
          currentSelection?.id === assetId &&
          currentSelection.isFavorite !== patch.isFavorite
        ) {
          setFavoriteCount((count) =>
            Math.max(0, count + (patch.isFavorite ? 1 : -1)),
          );
        }
        setPage((current) => ({
          ...current,
  ...
  const performFolderMove = async (
    ...
    try {
      setFolderMovePlan(
        await localLibraryApi.planFolderMove(
          target.relativePath,
          destinationParent,
          topLevelName,
          "skip",
  ...
  const executeFolderMovePlan = async () => {
    ...
    setFolderOperationBusy(true);
    try {
      const result = await localLibraryApi.executeFolderMovePlan(
        folderMovePlan.id,
      );
  ...
  const organizeFolder = async (
    ...
    const currentParent = target.relativePath.includes("/")
  ...
  const finishActiveFolderRemoval = async (permanent: boolean) => {
    ...
    try {
      if (permanent)
        await localLibraryApi.permanentlyDeleteActiveFolder(
          deleteFolderTarget.relativePath,
        );
      else await localLibraryApi.deleteFolder(deleteFolderTarget.relativePath);
      if (
        folder === deleteFolderTarget.relativePath ||
        folder.startsWith(`${deleteFolderTarget.relativePath}/`)
      ) {
        setFolder("");
      }
      toast.success(
        permanent ? copy.permanentlyDeletedFolder : copy.folderDeleted,
      );
  ...
  const restoreFolderBatch = async (
    destinationParent: string,
    topLevelName: string,
  ) => {
    ...
    try {
      await localLibraryApi.restoreFolder(
        restoreFolderTarget.id,
        destinationParent,
        topLevelName,
      );
  ...
  const copyAssetToClipboard = useCallback(
    async (asset: LocalAsset, cut: boolean) => {
      try {
  ...
  const uploadAsset = useCallback(
    (asset: LocalAsset) => {
      if (
        ...
        return;
      useUploadIntentStore
        .getState()
        .enqueue({ source: "local-assets", assetIds: [asset.id] });
  ...
  const queueSelectedAssetUpload = useCallback(
    async (settings: UploadSettings) => {
      if (
        fileOperationsBlockedByQuery ||
        !isAuthenticated ||
        uploadPreparingRef.current ||
      ...
      try {
        const prepared = await PrepareLocalAssetUpload(
          selectedUploadAssets.map((asset) => asset.id),
        );
        const ready = (prepared || []).filter((file) => file && !file.error);
        const failedCount = Math.max(
          0,
          selectedUploadAssets.length - ready.length,
        );
        if (ready.length === 0) {
        ...
        if (failedCount > 0)
          toast.warning(
            copy.batchUploadPartial
              .replace("{success}", ready.length.toLocaleString())
              .replace("{failed}", failedCount.toLocaleString()),
          );
        else toast.success(`${copy.uploadQueued} (${ready.length})`);
        clearAssetSelection();
      } catch (error) {
        toast.error(
          error instanceof Error ? error.message : copy.batchUploadFailed,
        );
      } finally {
    ...
    [
      addTasks,
      clearAssetSelection,
      copy.batchUploadFailed,
      copy.batchUploadPartial,
      copy.noStorageSources,
      copy.uploadQueued,
      fileOperationsBlockedByQuery,
      isAuthenticated,
      selectedUploadAssets,
    ],
  );

  const uploadSelectedToStorage = useCallback(
    (storageSourceId: string) => {
      void queueSelectedAssetUpload({
        ...quickUploadSettings,
        storageSourceId,
      });
    },
  ...
  const openSelectedUploadSettings = useCallback(() => {
    if (!uploadPreparing && selectedUploadAssets.length > 0)
      setBatchUploadSettingsOpen(true);
  }, [selectedUploadAssets.length, uploadPreparing]);

  const confirmSelectedUploadSettings = useCallback(
  ...
  const queueLocalAssetUpload = useCallback(
    async (asset: LocalAsset, settings: UploadSettings) => {
      if (
        !isAuthenticated ||
        uploadPreparingRef.current ||
        asset.availability !== "active" ||
      ...
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : copy.uploadPrepareFailed,
        );
      } finally {
        uploadPreparingRef.current = false;
        setUploadPreparing(false);
    ...
    [
      addTasks,
      copy.noStorageSources,
      copy.uploadPrepareFailed,
      copy.uploadQueued,
      isAuthenticated,
    ],
  );

  const openUploadSettings = useCallback(
    (asset: LocalAsset) => {
      if (
        !uploadPreparing &&
        asset.availability === "active" &&
        ...
        setUploadSettingsAsset(asset);
    },
    [uploadPreparing],
  );

  const confirmUploadSettings = useCallback(
    (settings: UploadSettings) => {
  ...
  const uploadAssetToStorage = useCallback(
    (asset: LocalAsset, storageSourceId: string) => {
      void queueLocalAssetUpload(asset, {
        ...quickUploadSettings,
        storageSourceId,
      });
    },
  ...
  const openSystem = async (asset: LocalAsset) => {
    try {
      await localLibraryApi.openInDefaultApp(asset.id);
    } catch (error) {
      toast.error(parseLocalLibraryError(error).message);
    }
  };
  ...
  const openRenameAsset = useCallback((asset: LocalAsset) => {
    if (asset.availability === "active")
      setAssetFileOperation({ mode: "rename", asset });
  }, []);
  ...
  const openMoveSelectedAssets = useCallback(() => {
    const activeIds = selectedAssetIds.filter(
      (id) =>
        page.items.find((item) => item.id === id)?.availability === "active",
    );
  ...
  const performAssetMove = async (
    ...
    setAssetFileOperationBusy(true);
    try {
      const plan = await localLibraryApi.planAssetMove(
        assetIds,
        destinationFolder,
        conflictPolicy,
      );
      ...
      if (failedIds.length === 0) clearAssetSelection();
        ...
        selectAsset(
          page.items.find((item) => item.id === failedIds[0]) ?? null,
        );
      }
      await Promise.all([reloadFolders(), refreshSnapshot()]);
      refreshAssets();
  ...
  const executeAssetMovePlan = async () => {
    ...
    setAssetFileOperationBusy(true);
    try {
      const execution = await localLibraryApi.executeAssetMovePlan(
        assetMovePlan.id,
      );
      runResultsMessage(
        execution.results,
  ...
  const moveAssets = async (
    ...
    await performAssetMove(
      assetFileOperation.assetIds,
      destinationFolder,
      conflictPolicy,
      false,
    );
  ...
  const applyOrganizationDrop = async (
    assetIds: string[],
    target: { kind: "tag" | "collection" | "favorite"; id?: string },
  ) => {
    const activeIds = assetIds.filter(
      (id) =>
        page.items.find((item) => item.id === id)?.availability === "active",
    );
    ...
    try {
      if (target.kind === "tag" && target.id) {
        await localLibraryApi.batchUpdateAssetOrganization({
          assetIds: activeIds,
          addTagIds: [target.id],
        });
  ...
  const assetDropHandlers = (
    onDrop: (assetIds: string[]) => void,
    dropEffect: "move" | "link" = "move",
    targetFolder?: string,
  ) => ({
    onDragEnter: (event: ReactDragEvent) => {
      if (
        !event.dataTransfer.types.includes("application/x-mo-gallery-asset-ids")
      )
        return;
    ...
    onDragLeave: (event: ReactDragEvent) => {
      if (
        ...
        return;
      setAssetDropTargetFolder((current) =>
        current === targetFolder ? null : current,
      );
    },
  ...
  const canMoveFolderTo = (source: string, destinationParent: string) => {
    const currentParent = source.includes("/")
  ...
  const folderDropHandlers = (destinationParent: string) => ({
    onDragEnter: (event: ReactDragEvent) => {
      ...
      if (
        draggedFolderPath &&
        canMoveFolderTo(draggedFolderPath, destinationParent)
      )
        setFolderDropTarget(destinationParent);
    },
    onDragOver: (event: ReactDragEvent) => {
      if (
        event.dataTransfer.types.includes("application/x-mo-gallery-asset-ids")
    ...
    onDragLeave: (event: ReactDragEvent) => {
      ...
      if (
        ...
        setAssetDropTargetFolder((current) =>
          current === destinationParent ? null : current,
        );
        return;
      }
      if (!event.dataTransfer.types.includes(FOLDER_DRAG_TYPE)) return;
    ...
    onDrop: (event: ReactDragEvent) => {
      ...
      if (assetPayload) {
        ...
        try {
          ...
          if (
            ...
            void performAssetMove(ids, destinationParent, "skip", true);
        } catch {
          /* ignore malformed drag payload */
        }
        return;
      ...
      event.stopPropagation();
      const source =
        event.dataTransfer.getData(FOLDER_DRAG_TYPE) || draggedFolderPath;
  ...
  const renderFolderTree = (parentId = "", depth = 0): ReactNode =>
    foldersByParentId.get(parentId)?.map((item) => {
      ...
      const highlighted =
        assetDropTargetFolder === item.relativePath ||
        folderDropTarget === item.relativePath;
      const selected =
        ...
        <div key={item.id}>
          ...
          >
            <FolderContextTarget
              target={target}
              copy={copy}
              onCreate={setCreateFolderParent}
              onOpenInFileManager={(next) => void openFolderInFileManager(next)}
              onRename={(next) =>
                setOrganizeFolderTarget({ target: next, mode: "rename" })
              }
              onMove={(next) =>
                setOrganizeFolderTarget({ target: next, mode: "move" })
              }
              onProperties={setPropertiesFolder}
              onDelete={setDeleteFolderTarget}
            >
              <div
                ref={(node) => {
                  if (node)
                    folderTreeNodeRefs.current.set(item.relativePath, node);
              ...
              >
                ...
                >
                  ...
                  <span className="text-[9px] opacity-65">
                    {folderTreeAssetCounts.get(item.id) || 0}
                  </span>
                </button>
              </div>
            </FolderContextTarget>
          </div>
          {collapsed ? null : renderFolderTree(item.id, depth + 1)}
        </div>
      );
    }) || null;
  ...
  const executeBatchDelete = async () => {
    ...
    try {
      ...
      runResultsMessage(
        results,
        availability === "trashed"
          ? copy.batchPermanentDeleted
          : availability === "missing"
            ? copy.batchMissingRemoved
            : copy.batchTrashed,
  ...
  const permanentlyDelete = async () => {
    ...
    setDeleteBusy(true);
    try {
      const results = await localLibraryApi.permanentlyDeleteAssets([
        deleteAsset.id,
      ]);
  ...
  const deleteCloud = async () => {
    ...
    try {
      ...
      toast.error(
        parseLocalLibraryError(error).message || copy.cloudDeleteFailed,
      );
    } finally {
      setDeleteBusy(false);
    }
  ...
  const restoreAsset = async (asset: LocalAsset) => {
    ...
    try {
      ...
      toast.success(
        asset.trashEntryKind === "folder" ? copy.restoredFolder : copy.restore,
      );
  ...
  const recheckMissing = useCallback(
  ...
  const removeMissingRecord = async () => {
    ...
    setMissingMaintenanceBusy(true);
    try {
      const [result] = await localLibraryApi.removeMissingAssets([
        removeMissingAsset.id,
      ]);
  ...
  const saveOrganization = async (value: {
    ...
    try {
      if (organizationEditor.kind === "tag") {
        if (organizationEditor.item)
          await localLibraryApi.updateTag(
            organizationEditor.item.id,
            value.name,
            value.color,
          );
        ...
        if (organizationEditor.item)
          await localLibraryApi.updateCollection(
            organizationEditor.item.id,
            value.parentId,
            value.name,
            value.notes,
            organizationEditor.item.position,
          );
        else
          await localLibraryApi.createCollection(
            value.parentId,
            value.name,
            value.notes,
          );
      } else {
        await localLibraryApi.updateCollectionGroup(
          organizationEditor.item.id,
          value.parentId,
          value.name,
          organizationEditor.item.position,
  ...
  const deleteOrganization = async (deleteContents: boolean) => {
    ...
    setOrganizationBusy(true);
    try {
      if (organizationDelete.kind === "tag")
        await localLibraryApi.deleteTag(organizationDelete.id);
      else if (organizationDelete.kind === "collection")
        await localLibraryApi.deleteCollection(organizationDelete.id);
      else
        await localLibraryApi.deleteCollectionGroup(
      ...
      if (organizationDelete.kind === "tag")
        setTagIds(tagIds.filter((id) => id !== organizationDelete.id));
      if (organizationDelete.kind === "collection")
        setCollectionIds(
          collectionIds.filter((id) => id !== organizationDelete.id),
        );
      await reloadOrganization();
      refreshAssets();
    } catch (error) {
  ...
  const batchUpdateOrganization = useCallback(
    async (update: Omit<BatchAssetOrganizationUpdate, "assetIds">) => {
      if (selectedAssetIds.length < 2 && !selectedQueryToken) return;
      setOrganizationBusy(true);
      try {
        if (selectedQueryToken)
          await localLibraryApi.batchUpdateAssetOrganizationByQuery(
            selectedQueryToken,
            update,
          );
        else
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (
        ...
        batchUploadSettingsOpen ||
        createFolderParent ||
        deleteAsset ||
        deleteFolderTarget ||
        guideOpen ||
        organizeFolderTarget ||
        organizationDelete ||
        organizationEditor ||
        pendingImportPaths ||
  ...
  const setAssetTags = async (assetId: string, ids: string[]) => {
    setOrganizationBusy(true);
    try {
      await localLibraryApi.setAssetTags(assetId, ids);
      toast.success(copy.organizationUpdated);
      refreshAssets();
      await reloadOrganization();
    } catch (error) {
  ...
  const openAssetInFileManager = async (asset: LocalAsset) => {
    try {
      await localLibraryApi.openAssetInFileManager(asset.id);
    } catch (error) {
      toast.error(parseLocalLibraryError(error).message);
    }
  };
  ...
  const openFolderInFileManager = async (target: FolderTarget) => {
    try {
      await localLibraryApi.openFolderInFileManager(target.relativePath);
    } catch (error) {
      toast.error(parseLocalLibraryError(error).message);
    }
  };
  ...
  const setAssetCollections = async (assetId: string, ids: string[]) => {
    setOrganizationBusy(true);
    try {
      await localLibraryApi.setAssetCollections(assetId, ids);
      toast.success(copy.organizationUpdated);
      refreshAssets();
      await reloadOrganization();
    } catch (error) {
  ...
  const previewIndex = previewAsset
    ? page.items.findIndex((item) => item.id === previewAsset.id)
    : -1;
  const previewNeighbors = useMemo(() => {
    ...
    const previous = page.items
      ...
      .reverse()
      .find((item) => item.availability === "active");
  ...
  const previewNext = useCallback(async () => {
    ...
    try {
      ...
      const nextActive = nextItems.find(
    ...
    loadingMore,
    page.nextCursor,
    previewNeighbors.next,
    query,
    queryKey,
    setPreviewAsset,
  ]);
  const currentPathSegments = useMemo(() => {
    const rootSegments = snapshot.rootPath
      .split(/[\\/]+/)
      .filter(Boolean)
      .map((segment, index) =>
        index === 0 ? segment.replace(/:$/, "") : segment,
      );
  ...
  const hasStructuredFilters = Object.values(filters).some((value) =>
    Array.isArray(value)
      ? value.length > 0
      : value !== undefined && value !== "",
  );
  const canBrowsePhysicalFolders =
    availability === "active" &&
    !favoritesOnly &&
    tagIds.length === 0 &&
    collectionIds.length === 0 &&
  ...
  const currentChildFolders = useMemo(() => {
    ...
    const parentId = folder
      ? folders.find((item) => item.relativePath === folder)?.id
      : "";
  ...
  return (
    <div
      className="relative grid h-full min-h-0 grid-cols-[218px_minmax(0,1fr)_auto] grid-rows-[auto_minmax(0,1fr)] bg-background"
      onDragEnd={() => {
        setAssetDropTargetFolder(null);
        setDraggedFolderPath(null);
        setFolderDropTarget(null);
    ...
    >
      <header
        className="col-span-2 col-start-2 row-start-1 shrink-0 border-b"
      ...
      >
        <div className="flex h-16 items-center gap-4 px-4">
          ...
          <button
            type="button"
            data-local-library-guide="import"
            disabled={importBusy || isSuspended || isRepairRequired}
            onClick={chooseFiles}
            className="flex items-center gap-2 rounded-md px-3 py-2 text-xs font-medium disabled:cursor-wait disabled:opacity-70"
            style={{
          ...
          >
            ...
            {importBusy ? copy.importing : copy.import}
          </button>
          <button
            type="button"
            disabled={isSuspended || isRepairRequired}
            onClick={openBackups}
            className="flex items-center gap-2 rounded-md border px-3 py-2 text-xs hover:bg-secondary disabled:opacity-50"
          >
            <DatabaseBackup size={13} />
            {copy.databaseBackups}
          </button>
          <button
            type="button"
            data-local-library-guide="guide"
            onClick={() => setGuideOpen(true)}
            className="flex items-center gap-2 rounded-md border px-3 py-2 text-xs hover:bg-secondary"
          >
            <CircleHelp size={13} />
            {copy.guide.label}
          </button>
          <button
            ...
            className="rounded-md border px-3 py-2 text-xs hover:bg-secondary"
          >
            {copy.close}
          </button>
        </div>
      ...
      <div className="contents">
        <aside
          className="col-start-1 row-span-2 row-start-1 flex w-[218px] min-h-0 shrink-0 flex-col overflow-hidden border-r bg-card p-3"
          style={{ borderColor: "var(--border)" }}
        >
          <div className="custom-scrollbar min-h-0 flex-1 overflow-y-auto">
            <div data-local-library-guide="nav">
              <NavButton
                ...
                icon={Images}
                label={copy.allPhotos}
                count={snapshot.assetCount}
                onClick={() => {
                  setAvailability("active");
                  setFavoritesOnly(false);
                  setFolder("");
              ...
              <div
                ...
                {...assetDropHandlers(
                  (ids) =>
                    void applyOrganizationDrop(ids, { kind: "favorite" }),
                  "link",
                )}
              >
                <NavButton
                  active={availability === "active" && favoritesOnly}
                  icon={Heart}
            ...
            <div data-local-library-guide="folders">
              <button
                ...
                className="mb-2 mt-5 flex w-full items-center gap-1 rounded px-2 py-1 text-left text-[10px] font-medium uppercase tracking-[0.16em] hover:bg-secondary"
                ...
                <>
                  ...
                  >
                    <FolderContextTarget
                      ...
                      copy={copy}
                      onCreate={setCreateFolderParent}
                      onOpenInFileManager={(target) =>
                        void openFolderInFileManager(target)
                      }
                      onRename={() => undefined}
                      onMove={() => undefined}
                      onProperties={setPropertiesFolder}
                    ...
                    >
                      <button
                        ...
                        className="mb-0.5 flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-xs hover:bg-secondary"
                        style={{
                          backgroundColor:
                            assetDropTargetFolder === "" ||
                            folderDropTarget === ""
                              ? "var(--primary)"
                              : availability === "active" &&
                                  !favoritesOnly &&
            ...
            <OrganizationNavigation
              ...
              onDelete={setOrganizationDelete}
              onDropAssets={(ids, target) =>
                void applyOrganizationDrop(ids, target)
              }
            />
            <SidebarFilterSection
              ...
              icon={Palette}
              open={colorsOpen}
              onToggle={() => toggleSection("localColors")}
            >
                ...
                {SIDEBAR_COLORS.map((color) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        applySidebarFilter({
                          colorLabels: active ? undefined : [color],
                        })
                      }
            ...
            <SidebarFilterSection
              ...
              icon={Star}
              open={ratingsOpen}
              onToggle={() => toggleSection("localRatings")}
            >
              <div
                className="flex items-center gap-1 rounded-lg border bg-secondary/30 p-1"
                style={{
                  borderColor:
                ...
                {SIDEBAR_RATINGS.map((rating) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        applySidebarFilter(
                          active
                            ...
                            : {
                                ratingMin: rating,
                                ratingMax: rating === 0 ? 0 : undefined,
                              },
                        )
                      }
                      title={
                      ...
                      className="flex h-7 min-w-0 flex-1 items-center justify-center gap-0.5 rounded-md px-1 text-[10px] hover:bg-secondary"
                      style={{
                        backgroundColor: active ? "var(--accent)" : undefined,
          ...
          <div
            className="mt-3 shrink-0 rounded-lg border p-3 text-[10px] leading-4"
            style={{
          ...
          >
            <div className="grid grid-cols-3 gap-1 text-center">
              ...
              />
              <StatButton
                active={availability === "trashed"}
                value={snapshot.trashCount}
                label={copy.inTrash}
                onClick={() => {
                  setAvailability("trashed");
                  setFavoritesOnly(false);
                  setFolder("");
                }}
              />
        ...
        <main
          data-local-library-import-folder={
            canImportIntoCurrentView ? folder : undefined
          }
          style={
            canImportIntoCurrentView
              ? ({ "--wails-drop-target": "drop" } as CSSProperties)
              : undefined
          }
          className="relative col-start-2 row-start-2 flex min-h-0 min-w-0 flex-col overflow-hidden"
        >
          <div
            className="flex min-h-13 shrink-0 flex-wrap items-center gap-2 border-b bg-card/55 px-4 py-2.5 shadow-[0_1px_0_rgba(15,23,42,0.03)]"
            style={{
              borderColor: "color-mix(in srgb, var(--border) 78%, transparent)",
            }}
            data-local-library-guide="toolbar"
          >
            <div className="relative min-w-0 flex-[1.15]">
              ...
              <input
                ...
                className="h-9 w-full rounded-lg border bg-input/80 pl-9 pr-9 text-xs outline-none transition-shadow placeholder:text-muted-foreground/75 focus:ring-2 focus:ring-primary/20"
                style={{
                  borderColor:
              ...
              {search && (
                <button
                  type="button"
                  onClick={() => setSearch("")}
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 rounded p-1 transition-colors hover:bg-secondary"
                >
                  <X size={13} />
                </button>
              )}
            ...
            {page.total > page.items.length && (
              <button
                type="button"
                onClick={() => void selectAllQueryResults()}
                className="h-8 shrink-0 rounded-md border px-2.5 text-[10px] hover:bg-secondary"
              >
                {copy.selectAllResults} {page.total.toLocaleString()}
              </button>
            )}
            <label className="flex h-9 shrink-0 cursor-pointer items-center gap-2 rounded-lg border bg-input/65 px-2.5 text-[10px] transition-colors hover:bg-secondary/70">
              <input
                type="checkbox"
                checked={directFolderOnly}
                onChange={(event) => setDirectFolderOnly(event.target.checked)}
              />
              {copy.hideSubfolderPhotos}
            </label>
            <div className="flex h-9 shrink-0 items-center rounded-lg border bg-input/65 p-0.5">
              <button
                type="button"
                title={copy.croppedView}
                aria-label={copy.croppedView}
                onClick={() => setPhotoViewMode("crop")}
              ...
              <button
                type="button"
                title={copy.fittedView}
                aria-label={copy.fittedView}
                onClick={() => setPhotoViewMode("fit")}
                className="flex size-8 items-center justify-center rounded-md transition-colors hover:bg-secondary"
                style={{
                  backgroundColor:
              ...
              <button
                type="button"
                title={copy.masonryView}
                aria-label={copy.masonryView}
                onClick={() => setPhotoViewMode("masonry")}
                className="flex size-8 items-center justify-center rounded-md transition-colors hover:bg-secondary"
                style={{
                  backgroundColor:
            ...
            <button
              ...
              aria-label={
                sortDirection === "asc" ? copy.ascending : copy.descending
              }
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border bg-input/65 transition-colors hover:bg-secondary"
            >
              {sortDirection === "asc" ? (
                <ArrowUp size={13} />
              ) : (
          ...
          {(isSuspended || isRepairRequired) && (
            <div
              ...
              style={{
                borderColor: "var(--border)",
                color: "#b45309",
                backgroundColor: "color-mix(in srgb, #f59e0b 10%, var(--card))",
              }}
            >
              {scanLabel(snapshot, copy)}
            </div>
          )}
          {availability === "trashed" && (
            <FolderTrashSection
              entries={trashedFolders}
              copy={copy}
              loading={trashedFoldersLoading}
              busyId={trashFolderBusyId}
              onRestore={setRestoreFolderTarget}
              onPermanentDelete={setPermanentFolderTarget}
            />
          )}
          <LocalAssetGrid
            assets={page.items}
            ...
            copy={copy}
            emptyTitle={
              availability === "missing" ? copy.missingEmpty : undefined
            }
            emptyHint={
              availability === "missing" ? copy.missingEmptyHint : undefined
            }
            ...
            selectedIds={
              selectionMode
                ? [...new Set([...selectedAssetIds, ...existingAssetIds])]
                : selectedAssetIds
            }
            onSelect={selectGridAsset}
            onOpen={(asset) => {
              if (asset.availability === "active") setPreviewAsset(asset);
            }}
            onOpenFolder={openFolderFromContent}
            onLoadMore={loadMore}
            onOpenInFileManager={(asset) => void openAssetInFileManager(asset)}
            onClipboard={copyAssetToClipboard}
            onUpload={uploadAsset}
            onUploadSettings={openUploadSettings}
            onUploadToStorage={uploadAssetToStorage}
            ...
            onMove={openMoveAsset}
            onRestore={restoreAsset}
            onRetryPreview={retryPreview}
            onRecheckMissing={recheckMissing}
            onRemoveMissing={(asset) => {
              selectAsset(asset);
              setRemoveMissingAsset(asset);
            }}
          />
          {selectedCount > 0 && !selectionMode && (
            <LocalAssetSelectionToolbar
              ...
              busy={
                batchDeleteBusy || assetFileOperationBusy || uploadPreparing
              }
              ...
              canUpload={
                ...
                isAuthenticated &&
                selectedUploadAssets.length > 0 &&
                selectedUploadAssets.length === selectedAssetIds.length
              }
              canMove={
                !fileOperationsBlockedByQuery &&
                selectedAssets.length > 0 &&
                selectedAssets.length === selectedAssetIds.length &&
                selectedAssets.every((asset) => asset.availability === "active")
              }
              canDelete={
                !fileOperationsBlockedByQuery &&
                selectedAssets.length > 0 &&
              ...
              uploadHint={
                fileOperationsBlockedByQuery
                  ? copy.uploadSelectedUnavailable
                  : undefined
              }
              moveHint={
                fileOperationsBlockedByQuery
                  ? copy.moveSelectedUnavailable
                  : undefined
              }
              deleteHint={
                fileOperationsBlockedByQuery
                  ? copy.deleteSelectedUnavailable
                  : undefined
              }
              storageSources={storageSources}
              storageSourcesLoading={storageSourcesLoading}
          ...
          <div
            ...
            data-local-library-guide="statusbar"
          >
            <div className="flex min-w-0 flex-1 items-center gap-2 overflow-hidden whitespace-nowrap text-[11px]">
              <span
                className={`shrink-0 ${isScanning ? "animate-pulse" : ""}`}
                style={{
                  color: isScanning
                    ? "var(--primary)"
                    : "var(--muted-foreground)",
              ...
              <span
                ...
                style={{ color: "var(--muted-foreground)" }}
              >
                {snapshot.scan.lastPath || currentLocation}
              </span>
            </div>
            {snapshot.scan.state === "running" && (
              <>
                <button
                  disabled={scanBusy}
                  onClick={() => runScanAction("pause")}
                  className="flex items-center gap-1.5 rounded px-2 py-1 text-[10px] hover:bg-secondary"
                >
                  <Pause size={11} />
                  {copy.pause}
                </button>
            ...
            {snapshot.scan.state === "paused" &&
              !isSuspended &&
              !isRepairRequired && (
                <button
                  disabled={scanBusy}
                  onClick={() => runScanAction("resume")}
                  className="flex items-center gap-1.5 rounded px-2 py-1 text-[10px] hover:bg-secondary"
                >
                  <Play size={11} />
                  {copy.resume}
            ...
            {!isSuspended &&
              !isRepairRequired &&
              !["running", "paused"].includes(snapshot.scan.state) && (
                <button
                  disabled={scanBusy}
                  onClick={() => runScanAction("start")}
                  className="flex items-center gap-1.5 rounded px-2 py-1 text-[10px] hover:bg-secondary"
                >
                  <RefreshCw size={11} />
                  {copy.rescan}
            ...
            >
              ...
              <input
                ...
                step={8}
                value={gridSize}
                onChange={(event) => setPhotoGridSize(Number(event.target.value))}
                aria-label={copy.gridZoom}
                title={copy.gridZoom}
        ...
        <div className="col-start-3 row-start-2 min-h-0 overflow-hidden">
          {selectedAssetIds.length > 1 || selectedQueryToken ? (
            <LocalAssetBatchDetails
              selectedCount={
                selectedQueryToken
                  ? selectedQueryTotal
                  : selectedAssetIds.length
              ...
              collections={collections}
              copy={copy}
              busy={organizationBusy || assetFileOperationBusy}
              canMove={
                !selectedQueryToken &&
                selectedAssetIds.every(
                  (id) =>
                    page.items.find((item) => item.id === id)?.availability ===
                    "active",
                )
              }
              onClear={clearAssetSelection}
              onMove={openMoveSelectedAssets}
              onUpdate={(update) => void batchUpdateOrganization(update)}
            />
          ) : (
            <LocalAssetDetails
              asset={selectedAsset}
              copy={copy}
              saving={saving}
              maintenanceBusy={missingMaintenanceBusy || previewMaintenanceBusy}
              tags={tags}
              collections={collections}
              organizationBusy={organizationBusy}
              onSave={saveAsset}
              onPreview={(asset) => {
                if (asset.availability !== "missing") setPreviewAsset(asset);
              }}
              ...
              onRetryPreview={retryPreview}
              onRecheckMissing={recheckMissing}
              onRemoveMissing={(asset) => setRemoveMissingAsset(asset)}
              onSetTags={setAssetTags}
              onCreateTag={createTagFromDetails}
      ...
      {dropTargetFolder !== null && (
        <div
          className="pointer-events-none absolute inset-3 z-50 flex items-center justify-center rounded-xl border-2 border-dashed bg-background/90 backdrop-blur"
      ...
      {previewAsset && (
        <LocalLibraryPreview
          ...
          onOpenSystem={openSystem}
          hasPrevious={Boolean(previewNeighbors.previous)}
          hasNext={Boolean(previewNeighbors.next) || Boolean(page.nextCursor)}
          onPrevious={() =>
            previewNeighbors.previous &&
            setPreviewAsset(previewNeighbors.previous)
          }
          onNext={() => void previewNext()}
        />
      )}
      {backupDialogOpen && (
        <LocalLibraryBackupDialog
          copy={copy}
      ...
      {organizationEditor && (
        <OrganizationEditorDialog
          target={organizationEditor}
          groups={collectionGroups}
          copy={copy}
          busy={organizationBusy}
          onClose={() => setOrganizationEditor(null)}
          onSubmit={(value) => void saveOrganization(value)}
        />
      )}
      {organizationDelete && (
        <DeleteOrganizationDialog
          ...
          busy={organizationBusy}
          title={
            organizationDelete.kind === "tag"
              ? copy.deleteTagTitle
              : organizationDelete.kind === "collection"
                ? copy.deleteCollectionTitle
                : copy.deleteCollectionGroupTitle
          }
          body={
            organizationDelete.kind === "tag"
              ? copy.deleteTagBody
              : organizationDelete.kind === "collection"
                ? copy.deleteCollectionBody
                : organizationDelete.nonEmpty
                  ? copy.deleteCollectionGroupBody
                  : copy.deleteEmptyCollectionGroupBody
          }
          dangerousLabel={
            organizationDelete.kind === "group" && organizationDelete.nonEmpty
              ? copy.deleteGroupContents
              : copy.confirmPermanent
          }
          onClose={() => setOrganizationDelete(null)}
          onConfirm={() =>
            void deleteOrganization(Boolean(organizationDelete.nonEmpty))
          }
        />
      )}
      {pendingImportPaths && (
      ...
      {removeMissingAsset && (
        <RemoveMissingAssetDialog
          asset={removeMissingAsset}
          copy={copy}
          busy={missingMaintenanceBusy}
          onClose={() => setRemoveMissingAsset(null)}
          onConfirm={removeMissingRecord}
        />
      )}
      {deleteAsset && (
        <DeleteAssetDialog
          ...
          onRestore={() => void restoreAsset(deleteAsset)}
          onPermanent={permanentlyDelete}
      ...
      {organizeFolderTarget && (
        <MoveFolderDialog
          ...
          relativePath={organizeFolderTarget.target.relativePath}
          currentName={organizeFolderTarget.target.name}
          folders={folders}
          copy={copy}
          busy={folderOperationBusy}
          plan={folderMovePlan}
          ...
          onConfirm={organizeFolder}
          onExecute={() => void executeFolderMovePlan()}
        />
      )}
      {assetFileOperation && (
        <AssetFileOperationDialog
          mode={assetFileOperation.mode}
          asset={
            assetFileOperation.mode === "rename"
              ? assetFileOperation.asset
              : undefined
          }
          selectedCount={
            assetFileOperation.mode === "move"
              ? assetFileOperation.assetIds.length
              : 1
          }
          folders={folders}
          copy={copy}
          busy={assetFileOperationBusy}
          plan={assetMovePlan}
          onClose={() => {
            setAssetMovePlan(undefined);
            setAssetFileOperation(null);
          ...
          onMove={moveAssets}
          onConfirmPlan={() => void executeAssetMovePlan()}
        />
      )}
      {propertiesFolder && (
        <FolderPropertiesDialog
          ...
          loading={folderPropertiesLoading}
      ...
      {deleteFolderTarget && (
        <DeleteFolderDialog
          ...
          loading={folderDeletionPreviewLoading}
          error={folderDeletionPreviewError}
          busy={folderOperationBusy}
          onClose={() => setDeleteFolderTarget(null)}
          onTrash={() => void finishActiveFolderRemoval(false)}
          onPermanent={() => void finishActiveFolderRemoval(true)}
        />
      )}
      {restoreFolderTarget && (
        <RestoreFolderDialog
          entry={restoreFolderTarget}
          folders={folders}
          copy={copy}
          busy={trashFolderBusyId === restoreFolderTarget.id}
          onClose={() => setRestoreFolderTarget(null)}
          onConfirm={restoreFolderBatch}
        />
      )}
      {permanentFolderTarget && (
        <PermanentDeleteFolderDialog
          entry={permanentFolderTarget}
          copy={copy}
          busy={trashFolderBusyId === permanentFolderTarget.id}
          onClose={() => setPermanentFolderTarget(null)}
          onConfirm={permanentlyDeleteFolderBatch}
        />
      )}
      {uploadSettingsAsset && (
        <ImageUploadSettingsModal
          isOpen
          onClose={() => setUploadSettingsAsset(null)}
          onConfirm={confirmUploadSettings}
          pendingCount={1}
          t={t}
          token={token}
          initialSettings={quickUploadSettings}
          confirmLabel={copy.uploadNow}
        />
      )}
      {batchUploadSettingsOpen && (
        <ImageUploadSettingsModal
          isOpen
          onClose={() => setBatchUploadSettingsOpen(false)}
          onConfirm={confirmSelectedUploadSettings}
          pendingCount={selectedUploadAssets.length}
          t={t}
          token={token}
          initialSettings={quickUploadSettings}
          confirmLabel={copy.uploadNow}
  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent>
        ...
        <ContextMenuSeparator />
        <ContextMenuItem onSelect={() => onOpenInFileManager(target)}>
          <FolderSearch2 size={14} />
          {copy.openInFileManager}
        </ContextMenuItem>
        <ContextMenuItem onSelect={() => onCreate(target)}>
          <FolderPlus size={14} />
          {copy.newFolder}
        </ContextMenuItem>
        {target.isRoot ? null : (
          <>
            <ContextMenuItem onSelect={() => onRename(target)}>
              <FolderPen size={14} />
              {copy.renameFolder}
            </ContextMenuItem>
            <ContextMenuItem onSelect={() => onMove(target)}>
              <FolderInput size={14} />
              {copy.moveFolder}
            </ContextMenuItem>
          </>
        )}
        <ContextMenuItem onSelect={() => onProperties(target)}>
          <Info size={14} />
          {copy.folderProperties}
        </ContextMenuItem>
        {target.isRoot ? null : (
          <>
            <ContextMenuSeparator />
  ...
  return (
    <section className="mt-5">
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        className="mb-2 flex w-full items-center gap-1 rounded px-2 py-1 text-left text-[10px] font-medium uppercase tracking-[0.16em] hover:bg-secondary"
        style={{ color: "var(--muted-foreground)" }}
      >
        {open ? (
          <ChevronDown size={12} className="shrink-0" />
...
function StatButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={label}
      className="flex flex-col items-center gap-0.5 rounded-md px-1 py-1.5 hover:bg-secondary"
      style={{ color: "var(--muted-foreground)" }}
    >
      <span
