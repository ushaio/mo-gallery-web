...
function normalizeSnapshot(source: SnapshotSource): LibrarySnapshot {
  return {
    ...
    scan: {
      ...
      phase: source?.scan?.phase ? String(source.scan.phase) : undefined,
      current: Number(source?.scan?.current ?? 0),
      total: source?.scan?.total == null ? undefined : Number(source.scan.total),
      thumbnailCurrent: Number(source?.scan?.thumbnailCurrent ?? 0),
      thumbnailTotal: source?.scan?.thumbnailTotal == null ? undefined : Number(source.scan.thumbnailTotal),
...
export const localLibraryApi = {
  ...
  async listAssets(query: AssetQuery): Promise<AssetPage> {
    ...
    return {
      items: Array.isArray(source?.items) ? source.items.map((item) => ({
        ...
        livePhotoVideoMime: item.livePhotoVideoMime || undefined,
        livePhotoVideoUrl: item.livePhotoVideoUrl || undefined,
        livePhotoVideoLength: Number(item.livePhotoVideoLength ?? 0) || undefined,
  ...
  rebuildThumbnails: (mode: 'missing' | 'all') => RebuildLocalLibraryThumbnails(mode) as Promise<number>,
