...
import { FolderPropertiesDialog } from '../dialogs/FolderPropertiesDialog'
import { FolderTrashSection } from './FolderTrashSection'
import { ImportModeDialog } from '../dialogs/ImportModeDialog'
...
import { AssetFileOperationDialog } from '../dialogs/AssetFileOperationDialog'
import { AssetMoveConflictDialog } from '../dialogs/AssetMoveConflictDialog'
import { AddToCollectionConfirmDialog } from '../dialogs/AddToCollectionConfirmDialog'
import { OrganizationEditorDialog } from '../dialogs/OrganizationEditorDialog'
...
import { PermanentDeleteFolderDialog } from '../dialogs/PermanentDeleteFolderDialog'
import { RemoveMissingAssetDialog } from '../dialogs/RemoveMissingAssetDialog'
import { RepairThumbnailsDialog } from '../dialogs/RepairThumbnailsDialog'
...
import { FolderContextTarget, StatButton } from './WorkbenchSidebarParts'
...
export function LocalLibraryWorkbench({ copy, snapshot, onSnapshot, onClose, selectionMode = false, existingAssetIds = [], onSelectionChange }: Props) {
  ...
  // 后端仍按游标分页返回，前端在首屏之后自动把剩余页续抓完，
  // 不再暴露「加载更多」：网格始终持有当前查询的全部资产。
  ...
  // 首屏返回后立即渲染，随后在同一个请求批次内沿游标把剩余页续抓完，
  // 让网格最终持有当前查询的全部资产（滚动、范围选择、预览翻页都不再受页边界限制）。
  // requestId 让过期查询（换目录/筛选/会话）在任一页之间及时中止。
  useCachedPageEffect(() => {
    ...
    const applyOverrides = (items: LocalAsset[]) => {
    ...
    const drain = async () => {
      let result = await localLibraryApi.listAssets(query)
      if (requestId !== assetQueryRequestIdRef.current) return
      const items = applyOverrides(result.items)
      ...
      while (result.nextCursor) {
        ...
        result = await localLibraryApi.listAssets({ ...query, cursor })
        if (requestId !== assetQueryRequestIdRef.current) return
        const nextItems = applyOverrides(result.items)
        setPage((current) => ({ ...result, items: [...current.items, ...nextItems] }))
      }
    }
  ...
  // 全量加载后，预览的上一张/下一张只在已加载列表内移动，不再触发翻页取数。
  const previewNext = useCallback(() => {
    if (previewNeighbors.next) setPreviewAsset(previewNeighbors.next)
  }, [previewNeighbors.next, setPreviewAsset])
    ...
    <div className="relative grid h-full min-h-0 grid-cols-[218px_minmax(0,1fr)_auto] grid-rows-[auto_minmax(0,1fr)] bg-[color-mix(in_srgb,var(--background)_96%,var(--secondary))]" onDragEnd={() => { setAssetDropTargetFolder(null); setDraggedFolderPath(null); setFolderDropTarget(null) }}>
      ...
      <div className="contents">
        ...
        <main data-local-library-import-folder={canImportIntoCurrentView ? folder : undefined} style={canImportIntoCurrentView ? { '--wails-drop-target': 'drop' } as CSSProperties : undefined} className="relative col-start-2 row-start-2 flex min-h-0 min-w-0 flex-col overflow-hidden bg-[color-mix(in_srgb,var(--background)_94%,var(--secondary))]">
          ...
          <LocalAssetGrid assets={page.items} folders={currentChildFolders} loading={loading} total={page.total} copy={copy}
            ...
            selectedIds={selectionMode ? [...new Set([...selectedAssetIds, ...existingAssetIds])] : selectedAssetIds} focusedId={selectedAsset?.id} onSelect={selectGridAsset} onOpen={(asset) => { if (asset.availability === 'active') setPreviewAsset(asset) }} onOpenFolder={openFolderFromContent}
          ...
          <LibraryStatusBar data-local-library-guide="statusbar">
            <div className="flex min-w-0 flex-1 items-center gap-2 overflow-hidden whitespace-nowrap text-[11px]"><span className={`shrink-0 ${isScanning ? 'animate-pulse' : ''}`} style={{ color: isScanning ? 'var(--primary)' : snapshot.scan.state === 'failed' ? '#b45309' : scanLabel(snapshot, copy) === copy.scanDone ? '#5a8a5f' : 'var(--muted-foreground)' }}>{scanLabel(snapshot, copy)}</span>{loadingRest && <span className="flex shrink-0 items-center gap-1.5" style={{ color: 'var(--muted-foreground)' }}><Loader2 size={11} className="animate-spin" />{copy.loadingAll.replace('{loaded}', page.items.length.toLocaleString()).replace('{total}', page.total.toLocaleString())}</span>}{thumbnailsGenerating && !isScanning && <span className="flex shrink-0 items-center gap-1.5" style={{ color: 'var(--muted-foreground)' }}><Loader2 size={11} className="animate-spin" />{copy.thumbnailsGenerating}</span>}{['running', 'paused'].includes(snapshot.scan.state) && snapshot.scan.lastPath && <span className="min-w-0 flex-1 truncate" style={{ color: 'var(--muted-foreground)' }}>{snapshot.scan.lastPath}</span>}</div>
