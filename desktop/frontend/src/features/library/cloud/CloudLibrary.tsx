...
import { CloudAlbums } from "@/features/library/cloud/albums/CloudAlbums";
import { CloudFilmRolls } from "@/features/library/cloud/film-rolls/CloudFilmRolls";
import { CloudPhotos } from "@/features/library/cloud/photos/CloudPhotos";
...
function appApi(): AlbumAppAPI {
  const bridge = (
    window as unknown as { go?: { main?: { App?: AlbumAppAPI } } }
  ).go?.main?.App;
...
export function CloudLibrary({
  ...
  useEffect(() => {
    if (
      ...
      !loading &&
      albumId &&
      !albums.some((album) => album.id === albumId)
    )
      setAlbumId(null);
  ...
  const createAlbum = useCallback(() => {
    ...
    setFeatured(null);
    setSearchParams(
      { source: "cloud", view: "albums", create: "1" },
      { replace: true },
    );
  }, [
    setAlbumId,
  ...
  const manageAlbum = useCallback(
    (id: string, tab: AlbumDetailTab) => {
      ...
      setFeatured(null);
      setSearchParams(
        { source: "cloud", view: "albums", manage: id, tab },
        { replace: true },
      );
    },
    [
  ...
  const togglePublished = useCallback(
    async (album: Album) => {
      ...
      try {
        const updated = await appApi().UpdateAlbum(album.id, {
        ...
        toast.success(
          t(
            updated.isPublished
              ? "admin.album_published"
              : "admin.album_unpublished",
            language,
          ),
        );
      } catch (error) {
  ...
  return (
    <div className="flex h-full min-h-0 bg-[color-mix(in_srgb,var(--background)_96%,var(--secondary))]">
      <aside
        className="flex w-[218px] shrink-0 flex-col overflow-hidden border-r bg-card/82 p-3 shadow-[4px_0_18px_-20px_rgba(15,23,42,0.65)]"
        style={{
        ...
        <div className="custom-scrollbar min-h-0 flex-1 overflow-y-auto pr-1">
          ...
          <div className="mb-2 mt-5 flex items-center gap-1 px-1">
            ...
            <button
              ...
              className="flex min-w-0 flex-1 items-center gap-1.5 rounded px-1 py-1 text-left text-[10px] font-medium uppercase hover:bg-secondary"
              style={{
                color:
                  view === "albums"
                    ? "var(--foreground)"
                    : "var(--muted-foreground)",
                backgroundColor: showingAlbumBrowser
                  ? "var(--accent)"
                  : undefined,
              }}
            ...
            >
              ...
              <span className="truncate tracking-[0.16em]">
                {t("gallery.albums", language)}
              </span>
              <span
                className="ml-auto text-[9px] tabular-nums tracking-normal"
                style={{ color: "var(--muted-foreground)" }}
              >
                {albums.length}
              </span>
            ...
            <div className="space-y-0.5">
              ...
              {!loading && sortedAlbums.length === 0 && (
                <button
                  type="button"
                  onClick={createAlbum}
                  className="w-full rounded-md px-2.5 py-3 text-left text-[10px] hover:bg-secondary"
                  style={{ color: "var(--muted-foreground)" }}
                >
                  {t("admin.no_albums", language)} ·{" "}
                  {t("admin.create_album", language)}
                </button>
              )}
            </div>
          )}
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden bg-[color-mix(in_srgb,var(--background)_94%,var(--secondary))]">
        {view === "photos" ? (
          <CloudPhotos
      ...
      <SimpleDeleteDialog
        ...
        message={
          deleteTarget
            ? t("admin.album_delete_named_confirm", language).replace(
                "{name}",
                deleteTarget.name,
              )
            : ""
  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent>
        ...
        <ContextMenuSeparator />
        <ContextMenuItem onSelect={() => actions.onOpen(album.id)}>
          <FolderOpen size={14} />
          {t("admin.open_album", language)}
        </ContextMenuItem>
        <ContextMenuItem onSelect={() => actions.onEdit(album.id)}>
          <Pencil size={14} />
          {t("admin.edit_album", language)}
        </ContextMenuItem>
        <ContextMenuItem onSelect={() => actions.onManagePhotos(album.id)}>
          <Images size={14} />
          {t("admin.manage_photos", language)}
        </ContextMenuItem>
        <ContextMenuSeparator />
        <ContextMenuItem
          disabled={busy}
          onSelect={() => actions.onTogglePublished(album)}
        >
          {busy ? (
          ...
          {t(
            album.isPublished ? "admin.unpublish_album" : "admin.publish_album",
            language,
          )}
        </ContextMenuItem>
        <ContextMenuSeparator />
        <ContextMenuItem
          variant="destructive"
          onSelect={() => actions.onDelete(album)}
        >
          <Trash2 size={14} />
          {t("admin.delete_album", language)}
        </ContextMenuItem>
      </ContextMenuContent>
    </ContextMenu>
  );
...
function CloudAlbumsBrowser({
  ...
  return (
    <>
      ...
      <div className="custom-scrollbar min-h-0 flex-1 overflow-auto bg-[color-mix(in_srgb,var(--background)_94%,var(--secondary))] p-6">
        {loading ? (
          <div className="grid grid-cols-2 gap-5 lg:grid-cols-3 2xl:grid-cols-5">
            {Array.from({ length: 8 }, (_, index) => (
              <div
                key={index}
                className="aspect-[4/3] animate-pulse rounded-lg border bg-card/70"
                style={{
                  borderColor:
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-8 lg:grid-cols-3 2xl:grid-cols-5">
            {albums.map((album) => (
              <button
                key={album.id}
                type="button"
                onClick={() => onOpen(album.id)}
                className="group min-w-0 text-left outline-none transition-transform duration-200 hover:-translate-y-px active:translate-y-0 active:scale-[0.99]"
                title={`${album.name} · ${album.photoCount || 0} ${t("admin.photos", language)}`}
              >
                <div className="relative pt-3">
                  <span
                    className="absolute left-3 top-0 h-5 w-20 rounded-t-md border border-b-0 bg-muted transition-colors group-hover:bg-accent"
                  ...
                  >
                    {album.coverUrl ? (
                      <img
                        src={resolveAssetUrl(album.coverUrl)}
                        alt=""
                    ...
                    <span className="absolute bottom-2 left-2 right-2 flex items-end justify-between gap-2 text-white">
                      ...
                      <span className="shrink-0 rounded bg-black/45 px-1.5 py-0.5 text-[9px] tabular-nums backdrop-blur-sm">
                        {album.photoCount || 0}
                      </span>
                    </span>
                    {!album.isPublished && (
                      <span className="absolute right-2 top-2 flex items-center gap-1 rounded bg-black/50 px-1.5 py-0.5 text-[9px] text-white backdrop-blur-sm">
                        <EyeOff size={9} />
                        {t("admin.draft", language)}
                      </span>
                    )}
                  </span>
                </div>
                <span className="mt-2 block min-w-0 px-1">
                  ...
                  <span className="mt-0.5 flex items-center gap-1 truncate text-[10px] text-muted-foreground">
                    {album.location ? (
                      ...
                      <span>
                        {album.photoCount || 0} {t("admin.photos", language)}
                      </span>
                    )}
                  </span>
                </span>
