    ...
    return (
      <>
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          {activeTab === 'overview' ? (
            <AlbumOverview album={currentAlbum} onChange={setCurrentAlbum} language={language} />
          ) : !currentAlbum.id ? (
            <div className="flex flex-col items-center justify-center h-56 border border-dashed rounded-lg" style={{ borderColor: 'var(--border)', color: 'var(--muted-foreground)' }}>
              ...
              <p className="text-sm mb-4">{t('admin.album_name_required', language)}</p>
...
function AlbumOverview({ album, onChange, language }: { album: Album; onChange: (album: Album) => void; language: Locale }) {
  const update = (patch: Partial<Album>) => onChange({ ...album, ...patch })

  return (
    <div className="max-w-2xl space-y-5">
      <Field label={t('admin.album_name', language)}>
        <input
          value={album.name}
      ...
      <label className="flex items-start gap-3 p-4 rounded-lg border cursor-pointer" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--card)' }}>
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block">
      <span className="block mb-1.5 text-xs font-medium" style={{ color: 'var(--muted-foreground)' }}>{label}</span>
      {children}
    </label>
  )
}
