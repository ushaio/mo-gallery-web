...
const ZH_MUTATION = '(?:优化|排版|写入|填入|插入|添加|放入|放到|应用到|更新到|修改|调整|移动|替换|删除|裁剪|缩放|重排|重新排版)'
...
const EXPLICIT_EN_EDIT_PATTERNS = [
  ...
  /(?:improve|refine|modify|adjust|move|replace|delete|crop|resize|rearrange).{0,30}(?:the )?(?:current )?(?:spread|page|layout|image|photo|text|slot)/i,
  /(?:current )?(?:spread|page|layout|image|photo|text|slot).{0,30}(?:improve|refine|modify|adjust|move|replace|delete|crop|resize|rearrange)/i,
]
