...
/** 判断照片是否已被插入到内容中（按 data-photo-id 匹配） */
export function isPhotoInsertedInContent(content: string, photoId: string): boolean {
  if (!content) return false
  const pattern = new RegExp('\\b' + 'data-photo-id=(["\'])' + photoId + '\\1', 'i')
