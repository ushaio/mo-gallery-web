...
当前实现使用参数绑定的结构化 SQL、白名单排序、稳定 ID 次级排序和 cursor 分页。关键词查询目前是多个字段上的转义 `LIKE`，**尚未实现 FTS5**。

FTS5 是否随当前 `modernc.org/sqlite` 构建可靠可用、触发器/重建策略以及 100,000 资产下相对 `LIKE` 的收益，必须通过技术 Spike 后再启用。未完成这些验证前，不得把当前查询描述为全文检索。
...
- FTS5 可用性、索引同步和重建验证；
- 常用查询 P50/P95 与 `EXPLAIN QUERY PLAN` 报告；
...
- 未完成 FTS 与性能验证前，10 万资产查询目标仍不是已验收状态；
