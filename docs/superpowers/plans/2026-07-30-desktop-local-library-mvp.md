...
| Phase 5：回收、失联、备份与上传 | 大部分完成 | 回收/恢复/永久删除、失联维护和上传集成已完成；备份仅有底层工具草稿，尚未接入 Manager、Wails API 与 UI |
...
- [ ] 使用已验证一致性备份 API；
- [ ] 恢复前停止写入并备份当前 DB，恢复后 integrity check 和校准；
- [ ] UI 明确备份不含原图与缓存。
