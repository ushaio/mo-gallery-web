...
| 阶段 | 当前状态 | 代码事实 |
|---|---|---|
| Phase 0：阻塞性技术 Spike | 未完成 | SQLite、媒体、watcher 和锁的当前实现选型已记录到 ADR-0038～0040；正式样本库、FTS/10 万项基准、真实介质和干净 Windows VM 验证尚缺 |
...
| Phase 5：回收、失联、备份与上传 | 基本完成 | 回收/恢复/永久删除、失联维护和上传集成已完成；每日变更备份、升级前备份、手动备份、恢复前保护、完整性检查及恢复 UI 已接入 |
...
2. 查询性能收尾：FTS/索引、`EXPLAIN QUERY PLAN`、10 万资产 P50/P95 与冷暖缓存基准。
3. ADR-0038～0040 中列出的 FTS、真实媒体样本、NTFS/exFAT、Windows 10/11 干净 VM、Wails production build、故障注入、无障碍及截图矩阵。
...
- [x] 新建 `ADR-0040` 记录 watcher、路径策略、锁选型及未验证边界。
...
- [x] 升级前一致性备份，备份失败阻断迁移，迁移失败自动恢复升级前数据库；
...
- [x] 日变更备份 7 份、升级前备份 3 份、立即备份；
- [x] 使用已验证一致性备份 API；
- [x] 恢复前停止写入并备份当前 DB，恢复后 integrity check 和校准；
- [x] UI 明确备份不含原图与缓存。

> 2026-07-31 实现说明：已提供每次成功打开后的首次数据变更日备份（保留 7 份）、schema 升级前备份（保留 3 份，备份失败阻断迁移，迁移失败恢复升级前数据库）、备份列表、立即备份和从指定备份恢复。备份通过 SQLite online backup API 生成并执行完整性检查；恢复前自动创建 `pre-restore` 备份、停止 watcher/扫描/派生图任务并替换数据库，随后以新 session 重开并启动校准扫描。
...
## 下一阶段 V1.1：专业资产管理与发布 Gate

依据：ADR-0041、ADR-0042。目标不是继续无边界扩展功能，而是完成可靠性证据并收尾专业照片管理工作流。

### V1.1-P0：发布可靠性

- [ ] Windows 10/11 x64 干净 VM 安装、启动、升级和卸载；
- [ ] NTFS 与直连 exFAT 外接卷建库、扫描、断开、重连和整体迁移；
- [ ] 10 万项混合真实照片库首屏、查询、扫描、缩略图吞吐、峰值 RAM/CPU 和缓存体积基准；
- [ ] P0 第一批优先完成文件操作恢复：强制退出、磁盘/SQLite 部分成功、外部竞态、重开重放和 repair_required；
- [ ] 随后覆盖 watcher 事件风暴、漏事件、临时文件、同路径覆盖和最终校准；
- [ ] 再覆盖 SQLite busy/corrupt/disk full、卷断开和迁移恢复；
- [ ] 目录穿越、链接 TOCTOU、伪造资产 ID、旧 session、保留目录和危险默认焦点；
- [ ] 真实媒体样本采用本地私有样本清单，不提交原文件；缺少证据的格式标记为未验证；
- [ ] 键盘等价操作、颜色非唯一表达、缩放、主题、语言和 reduced motion。

### V1.1-P1：专业照片管理收尾

实施优先级：原图查看 > 文件管理器混合浏览 > 批量整理/查询 > 通用文件操作计划。

- [ ] 原图查看：双击先进入屏幕预览，再显式进入原图/100%；按当前查询结果前后切换；工具栏 + 滚轮缩放 + 拖动画布；失败保留预览并提示；
- [ ] 文件管理器混合浏览：真实目录始终显示直接子文件夹，递归或直接照片与文件夹混合排列，双击/Enter 进入，目录切换回到顶部；
- [ ] 批量整理、筛选摘要、详情栏和快捷键行为统一；
- [ ] 通用文件操作计划：冲突策略、执行前复核、逐项结果、持久化恢复日志；
- [ ] 文件夹操作展开后代冲突，禁止覆盖并保持资产身份不变量。
...
- [ ] Zine、故事、AI、跨库搜索/拖放和正式 macOS 发布。

**V1.1 Gate：** P0 证据齐全且关键安全/数据不变量通过后，才可把 P1 作为正式版本能力对外发布；P2 必须重新访谈并建立独立 ADR。

### 2026-08-02 执行记录

- [x] 建立私有媒体样本协议、模板清单、生成器和不提交原文件的证据规则：`desktop/testdata/local-library/README.md`、`manifest.template.json`、`generate_manifest.py`。
- [x] 生成器按稳定样本 ID 合并清单，保留未变字节的人工字段；哈希变化时重置验证字段，缺失样本默认拒绝删除并需显式 `--prune`。
- [x] 建立 V1.1 发布 Gate 报告模板：`docs/requirements/desktop-local-library/v11-release-gate-report-template.md`。
- [x] 原图查看器补充工具栏无障碍名称、滚轮/键盘缩放、缩放重置、拖动状态、焦点圈定/恢复，以及原图失败时保留屏幕预览、明确原因和可重试入口。
- [x] 真实目录混合浏览、双击/Enter 进入和目录切换回顶已落地，见 ADR-0042 和本地资源库网格实现。
- [ ] Go/前端自动验证：当前工作环境缺少 `go` 和 `node`，待具备工具链后运行完整测试与构建。
- [ ] Windows 10/11、NTFS/exFAT、私有真实媒体和 10 万项性能证据仍未验证，不得提前宣称正式发布支持。
