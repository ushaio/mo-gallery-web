...
export function HomeContent({ initialPhotos }: HomeContentProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentHeroIndex((prev) => (prev + 1) % initialPhotos.length)
    }, 8000)
    return () => clearInterval(interval)
  }, [initialPhotos.length])

  const heroImage = initialPhotos.length > 0 ? initialPhotos[currentHeroIndex] : null
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground selection:bg-primary/30 selection:text-primary">
      ...
      >
        ...
        {initialPhotos.length > 1 && (
          ...
          >
            {initialPhotos.slice(0, 5).map((_, i) => (
              <button
                key={i}
      ...
      <section className="px-6 md:px-12 lg:px-24 pb-32 pt-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {initialPhotos.slice(0, 6).map((image, index) => (
            <motion.div
              key={image.id}
