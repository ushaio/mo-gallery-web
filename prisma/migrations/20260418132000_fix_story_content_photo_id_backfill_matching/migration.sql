-- Fix story content photo-id backfill matching for PostgreSQL regex semantics.
--
-- This migration normalizes internal gallery images to:
--   <img data-photo-id="PHOTO_ID" ...>
-- and removes src for matched internal images.
--
-- It handles:
-- 1. HTML <img src="..."> tags with absolute or relative URLs
-- 2. Markdown image syntax ![alt](url) and ![alt](url =480x)
--
...
BEGIN
  ...
  WITH raw_candidates AS (
    SELECT "id" AS photo_id, btrim("url") AS candidate
    FROM "Photo"
    WHERE "url" IS NOT NULL AND btrim("url") <> ''

    UNION ALL

    SELECT "id" AS photo_id, btrim("thumbnailUrl") AS candidate
    FROM "Photo"
    WHERE "thumbnailUrl" IS NOT NULL AND btrim("thumbnailUrl") <> ''
  ),
  normalized_candidates AS (
    SELECT photo_id, candidate
    FROM raw_candidates

    UNION ALL

    SELECT photo_id, regexp_replace(candidate, '^(https?:)?//[^/]+', '')
    FROM raw_candidates
    WHERE candidate ~* '^(https?:)?//'

    UNION ALL

    SELECT photo_id, regexp_replace(regexp_replace(candidate, '^(https?:)?//[^/]+', ''), '^/+', '')
    FROM raw_candidates
    WHERE candidate ~* '^(https?:)?//'

    UNION ALL

    SELECT photo_id, regexp_replace(candidate, '^/+', '')
    FROM raw_candidates
  ),
  ...
  FOR story_rec IN
    ...
    FROM "Story"
    WHERE "content" ILIKE '%<img%' OR "content" LIKE '%![%'
    ORDER BY "updatedAt" DESC
  LOOP
    next_content := story_rec."content";

    -- Normalize HTML <img> tags to data-photo-id only when src matches Photo.url/thumbnailUrl.
    FOR html_match IN
      WITH story_tags AS (
        SELECT
          tag_match[1] AS full_tag,
          (regexp_match(tag_match[1], $imgsrc$src=(["'])(.*?)\1$imgsrc$, 'i'))[2] AS src
        FROM regexp_matches(next_content, $imgtag$(<img[^>]*>)$imgtag$, 'gi') AS tag_match
      )
      SELECT DISTINCT
        story_tags.full_tag,
        story_tags.src,
    ...
    LOOP
      next_tag := regexp_replace(
        html_match.full_tag,
        $srcattr$\s*src=(["'])(.*?)\1$srcattr$,
        '',
        'i'
      );

      IF next_tag ~* 'data-photo-id\s*=' THEN
        next_tag := regexp_replace(
          next_tag,
          $photoid$data-photo-id\s*=\s*(["'])(.*?)\1$photoid$,
      ...
      next_tag := regexp_replace(next_tag, '\s+>', '>', 'g');
    ...
    -- Convert Markdown images to data-photo-id-only HTML img tags.
    FOR md_match IN
  ...
  RAISE NOTICE 'Story content photo-id backfill matching fix complete.';
