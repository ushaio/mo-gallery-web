...
- Shared `@mo-gallery/*` packages come from [mo-gallery-shared](https://github.com/ushaio/mo-gallery-shared) and are consumed as a workspace copy: this repo keeps a `packages/*` mirror at its root (a pnpm workspace member via `pnpm-workspace.yaml`) and depends on it through `workspace:*` (e.g. `"@mo-gallery/ai-agent": "workspace:*"`), with no git dependencies.
- Polyrepo layout: this repository is only the web app. The desktop client ([emulsion-desktop](https://github.com/ushaio/emulsion-desktop)), the Flutter client ([emulsion-app](https://github.com/ushaio/emulsion-app)), and the shared packages ([mo-gallery-shared](https://github.com/ushaio/mo-gallery-shared)) are independent checkouts at the sibling folders `../emulsion-desktop`, `../emulsion-app`, and `../mo-gallery-shared` — the individual shared packages live under `../mo-gallery-shared/packages/*`, which is the only editable source; this repo's `packages/*` is a generated mirror of it. They are separate git repositories — commit inside the relevant folder, never from here.
- Consuming shared packages locally: run `pnpm sync` in `../mo-gallery-shared` to mirror its `packages/*` into this repo's `packages/*`. Do not edit the mirrored files here — the sync has drift detection (`.mo-gallery-shared-sync.json`) and will report/refuse manual changes — and do not add `file:`/`link:` dependencies.
